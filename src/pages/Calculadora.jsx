...
import { ChevronDown, ChevronUp, Settings, RotateCcw, FileText, Plus, X } from 'lucide-react'
...
// ── Precios base (persisten en localStorage) ──────────────────────────────────
const DEFAULT_CONFIG = {
  filamento_rollo_precio: 90000,  // COP por rollo 1 kg
  filamento_rollo_gramos: 1000,   // gramos del rollo
  anillo_12mm_precio:     100,
  anillo_20mm_precio:     400,
  anillo_cadena_precio:   500,
  resina_litro_precio:    80000,  // COP por litro A+B
  tarifa_hora:            15000,  // COP/hora mano de obra
}
...
const DEFAULT_RECETA = {
  ...
  resina_ml:   0,
  tiempo_min:  0,
  margen:      40,   // % markup sobre costo
  comision:    25,   // % comision del vendedor sobre utilidad bruta
}

const ANILLO_TIPOS = [
  { id: '12mm',   label: 'Anillo 12 mm' },
  { id: '20mm',   label: 'Anillo 20 mm' },
  { id: 'cadena', label: 'Con cadenita' },
]

// ── Fila de resultado ─────────────────────────────────────────────────────────
function Fila({ label, valor, muted, bold, green, red, amber, dashed }) {
  return (
    <div className={`flex items-center justify-between py-1.5 ${dashed ? 'border-t border-dashed border-[#e2e6ea] mt-1 pt-2.5' : ''}`}>
      <span className={`text-xs ${muted ? 'text-[#8a9ab0]' : 'text-navy-600'} ${bold ? 'font-semibold' : ''}`}>{label}</span>
      <span className={`text-sm font-semibold tabular-nums
        ${green ? 'text-green-600' : red ? 'text-red-500' : amber ? 'text-amber-600' : 'text-navy-600'}`}>
        {valor}
      </span>
    </div>
  )
...
export default function Calculadora() {
  ...
  // ── Anillos helpers ────────────────────────────────────────────────────────
  function addAnillo() {
    setRec(r => ({ ...r, anillos: [...r.anillos, { tipo: '12mm', cantidad: 1 }] }))
  }
  function removeAnillo(i) {
    setRec(r => ({ ...r, anillos: r.anillos.filter((_, j) => j !== i) }))
  }
  function updAnillo(i, key, val) {
    setRec(r => ({ ...r, anillos: r.anillos.map((a, j) => j === i ? { ...a, [key]: val } : a) }))
  }
  ...
  const calc = useMemo(() => {
    ...
    const costoAnillos = rec.anillos.reduce(
      (s, a) => s + precioAnillo(a.tipo) * a.cantidad, 0
    )
    ...
    const gananciaAndres  = utilidadBruta * (rec.comision / 100)
    const gananciaDimas   = utilidadBruta - gananciaAndres

    const gananciaAndresTotal = gananciaAndres * rec.cantidad
    const gananciaDimasTotal  = gananciaDimas  * rec.cantidad

    return {
      costoFilamento, costoAnillos, costoResina, costoTiempo,
      costoXUnidad, costoTotal,
      precioSugerido, precioTotal,
      utilidadBruta, gananciaAndres, gananciaDimas,
      gananciaAndresTotal, gananciaDimasTotal,
      precioAnillo,
    }
  }, [rec, config])
    ...
    <div className="p-4 lg:p-6 max-w-5xl mx-auto">
      ...
      <div className="grid lg:grid-cols-2 gap-5">
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label className="block text-xs font-medium text-navy-600 mb-1">Nombre</label>
                <input
              ...
              <div>
                <label className="block text-xs font-medium text-navy-600 mb-1">Cantidad</label>
                <div className="relative">
                  <input type="number" min={1} value={rec.cantidad}
                    onChange={e => updRec('cantidad', Math.max(1, Number(e.target.value)))}
                    className="w-full border border-[#e2e6ea] rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30" />
              ...
              <div>
                ...
                <div className="relative">
                  <input type="number" min={0} max={500} value={rec.margen}
            ...
            <div className="flex flex-col gap-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  {calc.costoFilamento > 0 && <span className="text-xs text-accent font-semibold">{fmt(calc.costoFilamento)}/ud</span>}
                </div>
                <div className="relative">
                  <input type="number" min={0} step={0.5} value={rec.filamento_g}
                ...
                {rec.filamento_g > 0 && (
                  <p className="text-[11px] text-[#8a9ab0] mt-1">
                    {rec.filamento_g}g / {config.filamento_rollo_gramos}g · rollo {fmt(config.filamento_rollo_precio)}
                  </p>
                )}
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {calc.costoAnillos > 0 && <span className="text-xs text-accent font-semibold">{fmt(calc.costoAnillos)}/ud</span>}
                </div>

                {rec.anillos.length === 0 && (
                  <p className="text-xs text-[#8a9ab0] mb-2">Sin anillos — haz clic en "Agregar" para incluir uno</p>
                )}
                      ...
                      >
                        {ANILLO_TIPOS.map(t => (
                          <option key={t.id} value={t.id}>
                            {t.label} · {fmt(calc.precioAnillo(t.id))}
                          </option>
                        ))}
                      </select>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  {calc.costoResina > 0 && <span className="text-xs text-accent font-semibold">{fmt(calc.costoResina)}/ud</span>}
                </div>
                <div className="relative">
                  <input type="number" min={0} step={0.5} value={rec.resina_ml}
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  {calc.costoTiempo > 0 && <span className="text-xs text-accent font-semibold">{fmt(calc.costoTiempo)}/ud</span>}
                </div>
                <div className="relative">
                  <input type="number" min={0} step={5} value={rec.tiempo_min}
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Por unidad */}
          <Card className={cero ? 'opacity-60' : ''}>
            ...
            <div className="bg-[#f8f9fb] rounded-lg px-3 py-2 mb-3">
              {calc.costoFilamento > 0 && (
                <Fila label={`Filamento (${rec.filamento_g}g)`} valor={fmt(calc.costoFilamento)} muted />
              )}
              {rec.anillos.map((a, i) => (
                <Fila
                  key={i}
                  label={`${ANILLO_TIPOS.find(t => t.id === a.tipo)?.label} ×${a.cantidad}`}
                  valor={fmt(calc.precioAnillo(a.tipo) * a.cantidad)}
                  muted
                />
              ))}
              {calc.costoResina > 0 && (
                <Fila label={`Resina (${rec.resina_ml} ml)`} valor={fmt(calc.costoResina)} muted />
              )}
              {calc.costoTiempo > 0 && (
                <Fila label={`Tiempo (${rec.tiempo_min} min)`} valor={fmt(calc.costoTiempo)} muted />
              )}
              {cero && (
                <p className="text-xs text-[#8a9ab0] text-center py-1">Completa los materiales</p>
              )}
            </div>

            <Fila label="Costo de produccion" valor={fmt(calc.costoXUnidad)} bold />

            <div className="my-2 border-t border-dashed border-[#e2e6ea]" />

            <Fila label={`Precio sugerido (+${rec.margen}% margen)`} valor={fmt(calc.precioSugerido)} bold green />
            <Fila label="Utilidad bruta" valor={fmt(calc.utilidadBruta)} muted />

            {/* Ganancia de cada uno */}
            <div className="mt-2 rounded-lg border border-[#e2e6ea] overflow-hidden">
              <div className="grid grid-cols-2 divide-x divide-[#e2e6ea]">
                <div className="px-3 py-2.5 text-center">
                  <p className="text-[11px] text-[#8a9ab0] mb-0.5">Tu ganancia</p>
                  <p className="text-base font-bold text-green-600">{fmt(calc.gananciaDimas)}</p>
                </div>
                <div className="px-3 py-2.5 text-center">
                  <p className="text-[11px] text-[#8a9ab0] mb-0.5">Andres ({rec.comision}%)</p>
                  <p className="text-base font-bold text-amber-600">{fmt(calc.gananciaAndres)}</p>
                </div>
              </div>
            </div>
          </Card>
          ...
          {/* Para N unidades */}
          <Card className={`bg-navy-600 border-navy-600 ${cero ? 'opacity-60' : ''}`}>
            <p className="text-xs font-semibold uppercase tracking-wide text-white/60 mb-4">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <div className="bg-white/10 rounded-xl px-3 py-3 text-center">
                <p className="text-xs text-white/60 mb-1">Tu ganancia total</p>
                <p className="text-xl font-bold text-green-400">{fmt(calc.gananciaDimasTotal)}</p>
              </div>
              <div className="bg-white/10 rounded-xl px-3 py-3 text-center">
                <p className="text-xs text-white/60 mb-1">Andres gana</p>
                <p className="text-xl font-bold text-amber-300">{fmt(calc.gananciaAndresTotal)}</p>
              </div>
            </div>
          ...
          {/* Comision vendedor */}
          <Card>
            <p className="text-xs font-semibold uppercase tracking-wide text-[#8a9ab0] mb-3">Comision del vendedor</p>
            <label className="block text-xs font-medium text-navy-600 mb-1">
              % sobre utilidad bruta
              <span className="text-[#8a9ab0] font-normal ml-1">(Andres: 25% por defecto)</span>
            </label>
            <div className="relative">
              <input type="number" min={0} max={100} value={rec.comision}
        ...
        {showConfig && (
          <Card className="mt-3">
            <p className="text-xs text-[#8a9ab0] mb-4">
              Se guardan automaticamente en tu dispositivo. Actualiza cuando cambien los precios de tu proveedor.
            </p>
